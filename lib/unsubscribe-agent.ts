...
export class UnsubscribeAgent {
    ...
    try {
      const { text } = await generateText({
        ...
        prompt: `
          ...
          - HTML anchor tags with unsubscribe-related text
  ...
  private fallbackMultilingualExtraction(emailContent: string): UnsubscribeLink[] {
    ...
    const patterns = [
      ...
      {
        regex: /https?:\/\/[^\s<>"]+[^\s<>"]*(?=.*(?:dezabonare|aici))[^\s<>"]*/gi,
        lang: "ro",
      },
    ...
    // Look for general HTML anchor tags with href attributes
    const anchorPattern =
      /<a[^>]+href=["']([^"']+)["'][^>]*>([^<]*(?:unsubscribe|dezabonare|opt.out|remove|aici)[^<]*)<\/a>/gi
    while ((match = anchorPattern.exec(emailContent)) !== null) {
      links.push({
        url: match[1],
        text: match[2].trim(),
        method: "GET",
    ...
    try {
      ...
      const { text } = await generateText({
        ...
        prompt: `
          ...
          For Romanian emails, "aici" typically means "here" and is a direct link.
